...
export const copy = {
  ...
  refresh: "Refresh",
  stalenessTitle: "This value can go stale",
  stalenessDescription: "Utix is read-only and never submits transactions, so it cannot guarantee this sequence number is still current. If the account submits another transaction after this fetch, the value shown here is stale. Refresh immediately before using it to build a transaction.",
